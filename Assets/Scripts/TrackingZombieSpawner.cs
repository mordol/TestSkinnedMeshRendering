...
public class TrackingZombieSpawner : MonoBehaviour
{
    ...
    {
        // Directions for instance movement
        Vector3[] directions = new Vector3[spawnCount];
        for (int i = 0; i < spawnCount; i++)
        {
            directions[i] = new Vector3(UnityEngine.Random.Range(-1f, 1f), 0, UnityEngine.Random.Range(-1f, 1f)).normalized;
        }
        directionBuffer = new ComputeBuffer(spawnCount, sizeof(float) * 3);
        directionBuffer.SetData(directions);
        var kernel = animationFrameCompute.FindKernel(animationFrameComputeKernelName);
        animationFrameCompute.SetBuffer(kernel, "_Directions", directionBuffer);
        ...
        for (int x = -halfGridCount; x < halfGridCount; x++)
        {
            for (int z = -halfGridCount; z < halfGridCount; z++)
            {
                ...
                Quaternion rotation = Quaternion.FromToRotation(Vector3.forward, directions[spawnIndex]);
